-- =============================================
-- RUN ME: PGPASSWORD=By0N3-4r3NA psql -h localhost -U tesla -d byone_arena -f run_fix_report.sql
-- =============================================

-- 1. Add voucher_id to daily_rentals
ALTER TABLE daily_rentals ADD COLUMN IF NOT EXISTS voucher_id UUID REFERENCES vouchers(id);

-- 2. Backfill: update existing daily_rentals that used HEMAT50RB voucher
--    (cari rental dgn discount_amount > 0 yg belum punya voucher_id)
...
-- 3. Drop & recreate byoneCreateDailyRental (store voucher_id)
...
-- 4. Update byoneReportSummary: voucher usage from BOTH sessions AND daily rentals
--    + proper baseAmount + membership fields
...
-- 5. Update byoneDashboardSummary: include daily rental voucher count
...
BEGIN
    ...
    -- Voucher usage: session + daily rental
    voucher_agg AS (
        SELECT v.name AS voucher_name, v.code AS voucher_code,
            COUNT(*) AS usage_count, SUM(src.discount_amount) AS total_discount
        FROM (
            SELECT p.voucher_id, p.discount_amount
            FROM payments p WHERE p.payment_status = 'paid' AND p.voucher_id IS NOT NULL AND p.paid_at::DATE = p_date
            UNION ALL
            SELECT d.voucher_id, d.discount_amount
            FROM daily_rentals d WHERE d.voucher_id IS NOT NULL AND d.created_at::DATE = p_date
        ) src
        JOIN vouchers v ON v.id = src.voucher_id
